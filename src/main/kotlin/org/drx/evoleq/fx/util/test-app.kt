...
fun <D> showStage(stageComponent: FxComponent<Stage, D>): Parallel<Stub<D>> = Parallel {
...
@Suppress("unchecked_cast")
fun <D> launchStage(stageComponent: FxComponent<Stage,D>): Parallel<Stub<D>> = Parallel{
...
fun <P : Parent,D> showOnStage(parentComponent: FxComponent<P, D>): Parallel<Stub<D>> = Parallel{
    ...
    launchStage(stageComponent).get()
}

fun <N : Node, D> showNodeOnStage(nodeComponent: FxComponent<N, D>): Parallel<Stub<D>> = Parallel{
    val group = fxGroup<D>{
        tunnel()
        view{configure{}}
        child(nodeComponent)
